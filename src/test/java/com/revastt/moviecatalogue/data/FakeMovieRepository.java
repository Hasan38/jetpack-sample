...
public class FakeMovieRepository implements FakeMovieDataSource {
	...
	public static FakeMovieRepository getInstance(LocalRepository localRepository, RemoteRepository remoteData, AppExecutors appExecutors) {
		if (INSTANCE == null) {
			synchronized (FakeMovieRepository.class) {
				if (INSTANCE == null) {
					INSTANCE = new FakeMovieRepository(localRepository, remoteData, appExecutors);
				}
			}
		}
		return INSTANCE;
	...
	@Override
	public LiveData<Resource<PagedList<FavMovieEntity>>> getMoviePagedList() {
		return new NetworkBoundResource<PagedList<FavMovieEntity>, List<MoviesResult>>(appExecutors) {
			@Override
			protected LiveData<PagedList<FavMovieEntity>> loadFromDB() {
				return new LivePagedListBuilder<>(localRepository.getMoviePaged(), /* page size */ 20).build();
